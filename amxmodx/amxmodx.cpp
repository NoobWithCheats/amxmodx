...
/**
 * Перезагрукза текущего плагина
 */
// static cell AMX_NATIVE_CALL amx_reload_all_plugins(AMX *amx, cell *params)
// {
// 	enum {arg_count, arg_this}; // this - перезагружать текущий плагин?
// 	// UnloadAmxScript (amxx)
// }

/**
 * Перезагрузка всех плагинов
 */
// static cell AMX_NATIVE_CALL amx_reload_plugin(AMX *amx, cell *params)
// {
// 	// reload(amx); amx - плагин, который вызывает функию
// 	// UnloadAmxScript(amx)
// }

/**
 * Перезагрузка плагина по имени
 */
// static cell AMX_NATIVE_CALL amx_reload_plugin_name(AMX *amx, cell *params)
// {
// 	enum {arg_count, arg_name};
// 	// MF_GetScriptName
...
{					// *amx  char size_t   
	...
	g_plugins.unloadPlugin(pPlugin); //TODO: ссылка в ссылке
	int debugFlag;
	int search;

	// проверка, что плагин активен в plugins.ini или других plugins-*.ini
	if (g_plugins.SearchPluginInFile(get_localinfo("amxx_plugins", "addons/amxmodx/configs/plugins.ini"), pluginName, debugFlag))
	{
		search = 1;
	}
	else
	{
		...
		while (!files.empty())
		{
			...
			if (g_plugins.SearchPluginInFile(path, pluginName, debugFlag))
			{
				search = 1;
				break;
			}
	...
	*pPlugin = g_plugins.loadPlugin(get_localinfo("amxx_pluginsdir", "addons/amxmodx/plugins"), pluginName, error, sizeof(error), debugFlag);
		
	// ссылка, ссылка (1 и 2 арг). Это уже надо вызывать в amxmodx, иначе он не запишет себе эти плагины
	if (!g_plugins.registerPlugin(pPlugin, error, pluginName))
	{
		return false;
	}
